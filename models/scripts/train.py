...
"""
This reference code for LLaMA-3.1 1B training and inference was adapted from Andrej Karpathy, #4) work,
The code enables benchmarking LLama 3 on a system writhout a GPU. 
Several asserts have been commented out, the magic number does not match iin tinyshakespear headers
...
# 4) https://github.com/karpathy/llm.c/blob/master/train_llama3.py
...
def print_snap(str):
        ...
        pid = os.getpid()
        pmap_cmd = f"pmap -x {pid}"
        pmap = subprocess.run(pmap_cmd,shell=True,capture_output=True, text=True,check=True)
        pmap_out = pmap.stdout.splitlines(keepends=True)
        pmap_out = [x.strip('  ') for x in pmap_out]
        pmap_ll = pmap_out[-1].split()
        total =float( pmap_ll[-3])/1024
        rss = float(pmap_ll[-2])/1024
        dirty = float(pmap_ll[-1])/1024
        print(str, "total memory %.1f rss = %.1f, dirty = %.1f" % (total, rss, dirty))
        #base_memory_usage = process.memory_info().rss/1024/1024
        #print(str, " %s memory blocks: %.1f MiB rss = %.1f" % (len(stats), size /1024/ 1024, base_memory_usage))


def display_top(snapshot, str, key_type='lineno', limit=0):
    location = str
...
# AK's simple Distributed Data Loader
...
def _peek_data_shard(filename):
    ...
    if header[0] != 20240801:
        print(header[0])
        print("TODO ERROR: magic number mismatch in the data .bin file!")
        #exit(1)
...
def _load_data_shard(filename):
    with open(filename, "rb") as f:
        ...
        print(header[1]) #assert header[1] == 7, "unsupported version"
        ntok = header[2] # number of tokens (claimed)
        print(ntok)
        # the rest of it are tokens, stored as uint16
        tokens = np.frombuffer(f.read(), dtype=np.uint16) #was uint32
        print(len(tokens))
...
class LLaMA(nn.Module):
    ...
    def __init__(self,model_args):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            h = nn.ModuleList([Block(model_args) for _ in range(model_args.n_layer)]),
            ln_f = RMSNorm(model_args.n_embd, model_args.norm_eps),
        ))
        self.lm_head = nn.Linear(model_args.n_embd, model_args.vocab_size, bias=False)
        ...
        self.freqs_cis = precompute_freqs_cis(
            model_args.n_embd // model_args.n_head,
            model_args.block_size * 2,
    ...
    def from_pretrained_llama3_meta(cls, ckpt_dir, tokenizer_path):
        ...
        #torch.set_default_dtype(torch.FloatTensor)  # much faster loading
        print_snap("Before LLaMA call")

        model = LLaMA(model_args)
        
        print_snap("After LLaMA call")
        ...
        print_snap("After load checkpoint")
        
        #torch.set_default_dtype(torch.tensor([], dtype=original_default_type, device="cpu").type())  # restore default type
    ...
    def generate(
        ...
        total_len = min(8192, max_gen_len + max_prompt_len)

        pad_id = self.tokenizer.pad_id
        tokens = torch.full((bsz, total_len), pad_id, dtype=torch.long, device=device)
        for idx, t in enumerate(prompt_tokens):
            tokens[idx, : len(t)] = torch.tensor(t, dtype=torch.float, device=device)  # changed from long
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt_dir", type=str, default="/home/marc/.llama/checkpoints/Llama3.2-1B-Instruct/", help="llama3 model checkpoint (needed if use_hf=0)")
    parser.add_argument("--tokenizer_path", type=str, default="/home/marc/.llama/checkpoints/Llama3.2-1B-Instruct/tokenizer.model", help="path to llama3 tokenizer (needed if use_hf=0)")
    # file system input / output
    parser.add_argument("--input_bin", type=str, default="/home/marc/llm.c-master/dev/data/tinyshakespeare/tiny_shakespeare_val.bin", help="input .bin to train on")
    ...
    parser.add_argument("--sample_every", type=int, default=2, help="how often to sample from the model?")
    # debugging
    ...
    base_memory_usage = process.memory_info().rss/1024/1024
    print("before inference memory usage", base_memory_usage)
    print_snap("After parse args") 
   ...
   ## if master_process and args.write_tensors and (not args.inference_only):
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.overfit_single_batch:
            train_loader.reset()
            print("reset train loader")
        # micro-batch loop where we do gradient accumulation to reach desired total batch size
        lossf = 0.0 # for getting the mean loss (as simple float) over the accumulation steps
        for micro_step in range(grad_accum_steps):
            print_snap("Micro Step Start") 
            base_memory_usage = process.memory_info().rss/1024/1024
            print("before inference memory usage", base_memory_usage)
            command = f"sync; echo 3 > /proc/sys/vm/drop_caches"
            os.system(f'sudo sh -c "{command}"')
            time.sleep(3)
            base_memory_usage = process.memory_info().rss/1024/1024
            print("after = ", base_memory_usage)
            # fetch a batch
            x, y = train_loader.next_batch()
            x, y = x.to(device), y.to(device)
            print(x[:1,:10])
            ...
            with ctx:
                ...
                t2=time.time()
            fwd_memory_usage = process.memory_info().rss/1024/1024
            sn_after_fwd= tracemalloc.take_snapshot()
            print_snap("after forward Pass")
            # backward pass
            if not args.inference_only:
                loss.backward()
                t3=time.time()
            print_snap("after forward Pass")
            back_memory_usage = process.memory_info().rss /1024/1024
